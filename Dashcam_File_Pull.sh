...
# Script path
scriptpath= #replace with the path where your script will be stored.

# Root path
rootpath= #replace with your root path for your video files

#Folder path
folderpath= #replace with your root folder path

# Car Name
car= #car name for the files. 
...
ipaddress= #replace with your dashcams IP address on your network.
...
logfile=$(find "$folderpath" -mtime +${certain} -delete -print)
echo $(date) Removed old files:>>${scriptpath}oldfilecleanup.log
echo ${logfile} | tr ' ' '\n'.....>>${scriptpath}oldfilecleanup.log
logdir=$(find "$folderpath" -type d -empty -delete -print)
